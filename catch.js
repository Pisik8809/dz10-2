...
wolf.onmousedown = function(e) {
  ...
  function eggSave() {
    ...
    if ((wolfCoordX <= 700 && egg1.offsetLeft >= 120) && (wolfCoordY >= egg1CoordY && egg1CoordY >= 400)) {
        egg1.style.backgroundImage = 'url(ципа1.png)';
        localStorage.count = Number(localStorage.count) + 1;
        score.innerHTML = localStorage.count + 'egg';
        wolf.style.left = '850px';
        setTimeout(function() {
          egg1.style.visibility = 'hidden';
        }, 350);
    }
     if ((wolfCoordX <= egg2CoordX && wolfCoordX >= 875 && egg2CoordX <= 900) && (wolfCoordY >= egg2CoordY && egg2CoordY >= 400)) {
...
function chekWolf () {
  console.log(wolf.offsetLeft - 100);
}
setInterval(chekWolf, 500);
//console.log(egg1);
function egg1Move1() {
